...
// console.log(users);
  const handleDelete = (id) => {
    (async () => {
      const result = await RegisterSV.deleteregister(id);
  ...
  const handleStatus = (id) => {
    ...
    (async () => {
      const result = await RegisterSV.DeleteTrashRegister(id,user);
      if (result.data.status === true) {
                ...
                if (user.status === 0) {
                  return (
                    <tr className="datarow">
                      ...
                      <td>
                        <div className="name">
                          <Link href="brand_index.html"> {user.name}</Link>
                        </div>
...
export default DelteteTrashUser;
