...
const OrderExport = () => {
  ...
  const SaveKH = async (id) => {
    try {
      (async () => {
        const result = await RegisterSV.getById(id);
...
const handleQtyChange = (productId, event) => {
  const newQuantities = {
    ...quantities,
    [productId]: parseInt(event.target.value, 10) || 0,
  };
...
const handleSubmit = async (event) => {
 ...
 const handleRemoveProduct = (productId) => {
   ...
   const updatedProducts = productSelects.filter(
                        ...
                        <table className="table table-bordered table-striped">
                            ...
                            <tbody id="bodyproduct">
                                {productSelects && productSelects.map((product, index) => (
                                    <tr>
                                        <td>
                                            <img className="img-fluid" src={urlImageProduct + "/" + product.image} alt="h" />
                                        </td>
                                        <td>{product.name}</td>
